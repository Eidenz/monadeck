...
impl Monado {
	...
	/// Whether the loaded libmonado.so actually exports the controller-freeze
	/// symbol — i.e. a Monado fork that supports it. Stock/upstream Monado, where
	/// the optional symbol is absent, returns false. Lets callers hide UI that
...
pub trait ClientLogic: MonadoRef {
	...
	/// Freeze (hold in place) or unfreeze this client's hand-controller poses,
	/// while every other client keeps tracking the same controllers live.
	/// Requires a Monado fork exposing API >= 1.8.
	fn set_controller_freeze(&mut self, freeze: bool) -> Result<(), MndResult> {
		let monado = self.monado();
		unsafe {
			monado
				.api
				.mnd_root_set_client_controller_freeze(monado.root, self.id(), freeze as u32)
				.ok_or(MndResult::ErrorInvalidVersion)?
				.to_result()?;
		}
		Ok(())
