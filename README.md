...
- **Bring your desktop into VR.** Monitors become movable, resizable, curved screens driven by a real mouse, with a full VR keyboard, docking, saved layouts and a WayVR-style playspace drag. Captured straight into the GPU through PipeWire. Pick the size, curve, opacity and distance new screens start with, dim them or warm them up like a night light, and make B a middle click if you'd rather have that than the frozen-cursor click.
...
- **Play flat games in VR with your VR controllers.** Gaming mode turns the controllers into an Xbox pad every game already understands (rumble comes back as haptics), keeps the lasers off until the watch's mouse button asks for one, and blocks the VR app behind from seeing your inputs, so you can lie down in VRChat and play something else. Hang the screen in the world, let it trail your head, or hold it between your hands like a handheld. Per-game remaps to keys and mouse are edited in the desktop app (burger menu › Gamepad remaps). A VR game running behind would read that pad as its own gamepad: once Monadeck has seen a game running in VR it hides the pad from it on GE-style Protons, and on Valve's Proton the launch options the desktop app gives you to paste do the same (`SDL_GAMECONTROLLER_IGNORE_DEVICES=0x045e/0x028e`).
- **Let games talk to the overlay.** An OSC listener (off by default; Settings › OSC) lets VRChat avatar parameters, VRCOSC and friends toggle the watch, the minimal watch, the dashboard, screens and keyboard, or raise a toast: `/monadeck/watch`, `/monadeck/watch/mini`, `/monadeck/dashboard`, `/monadeck/screens`, `/monadeck/keyboard`, `/monadeck/gaming`, `/monadeck/letgo`, `/monadeck/notify "title" "body"`, or avatar parameters named `MonadeckWatch`, `MonadeckWatchMini`, `MonadeckDashboard`, `MonadeckScreens`, `MonadeckKeyboard`, `MonadeckGaming`, `MonadeckLetGo`.
- **Control the runtime without taking the headset off.** Nudge and rotate your play area, pick which app the headset shows, freeze an app's controllers, set a timer. With the Monado fork, a controller you switch off freezes the in-game hand in place; flip that to *Let go* (Settings › Controllers, System › Monado, a watch button or OSC) when you'd rather the game saw it switched off, e.g. so an avatar's own sleeping pose takes over. The choice is remembered.
- **See what's alive at a glance.** Controllers, trackers and gloves that are switched off turn grey on the deck, the watch and the bottom bar, and fade while they're out of tracking, like SteamVR. UdCap gloves show their battery too.
