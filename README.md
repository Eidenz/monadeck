...
- **Control the runtime without taking the headset off.** Nudge and rotate your play area, pick which app the headset shows, freeze an app's controllers, set a timer. With the Monado fork, a controller you switch off freezes the in-game hand in place; flip that to *Let go* (Settings › Controllers, a watch button or OSC) when you'd rather the game saw it switched off, e.g. so an avatar's own sleeping pose takes over.
