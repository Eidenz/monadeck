...
const BASE_URL: Option<&str> = option_env!("MD_TRANSLATE_BASE_URL");
const MODEL: Option<&str> = option_env!("MD_TRANSLATE_MODEL");
const API_KEY: Option<&str> = option_env!("MD_TRANSLATE_API_KEY");
...
pub fn translate_image(path: &Path) -> Result<String, String> {
    ...
    // Thinking models (Qwen3.x) otherwise spend the whole budget reasoning and
    // return empty content; llama.cpp honours `chat_template_kwargs`.
    ...
    let agent: ureq::Agent = ureq::Agent::config_builder().timeout_global(Some(Duration::from_secs(300))).build().into();
    let resp = agent
        .post(&url)
    ...
    let raw = v["choices"][0]["message"]["content"].as_str().ok_or("no content in response")?;
    // Strip any leaked <think>…</think> block.
    ...
    if text.is_empty() {
        let reasoning = v["choices"][0]["message"]["reasoning_content"].as_str().unwrap_or("");
        return Err(if reasoning.is_empty() {
            "empty translation".into()
        } else {
            "model only produced reasoning (thinking mode?) — no translation".into()
        });
    }
    Ok(text)
}
